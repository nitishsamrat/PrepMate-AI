...
function ProfileSetup() {
  ...
  const docsDone = Boolean(verification)
  const canContinue = photoDone && docsDone
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      formData.append('tenth', documents.tenth)
      formData.append('twelfth', documents.twelfth)
      formData.append('graduation', documents.graduation)
  ...
  const steps = [
    { id: 'photo', label: 'Profile photo', done: photoDone },
    { id: 'docs', label: 'Documents', done: docsDone },
    { id: 'ready', label: 'Ready to continue', done: canContinue },
  ]
  const activeIndex = steps.findIndex((s) => !s.done)

  return (
    <div className="relative min-h-full overflow-hidden bg-slate-50">
      <div className="pointer-events-none absolute inset-x-0 top-0 h-72 bg-gradient-to-b from-indigo-50 via-slate-50 to-transparent" />
      ...
      <div className="relative mx-auto max-w-5xl px-4 py-6 sm:px-6 lg:py-8">
        <div className="mb-6 max-w-2xl">
          ...
          <p className="mt-1.5 max-w-md text-sm leading-relaxed text-slate-600">
            Capture a live photo and verify your academic documents. Both are required before
            you can move on to interview preparation.
          </p>
        </div>
                ...
                <div className="flex flex-col items-start gap-1.5">
                  ...
                  <span
                    className={`text-[13px] font-medium ${
                      step.done || isActive ? 'text-slate-900' : 'text-slate-400'
                    }`}
                  >
                    {step.label}
                  </span>
                ...
                {index < steps.length - 1 && (
                  <div
                    className={`mx-4 mt-4 h-px flex-1 ${
                      steps[index + 1].done || step.done ? 'bg-emerald-500' : 'bg-slate-200'
                    }`}
                  />
                )}
              </li>
        ...
        <div className="grid items-stretch gap-4 lg:grid-cols-2">
          <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
            <div className="flex items-baseline justify-between border-b border-slate-100 px-5 py-3">
              <h2 className="text-sm font-semibold text-slate-900">01 — Profile photo</h2>
              {photoDone && <Check className="h-4 w-4 text-emerald-600" />}
            </div>
          ...
          <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
            <div className="flex items-baseline justify-between border-b border-slate-100 px-5 py-3">
              <h2 className="text-sm font-semibold text-slate-900">02 — Documents</h2>
              {docsDone && <Check className="h-4 w-4 text-emerald-600" />}
            </div>
            <div className="p-4 sm:p-5">
        ...
        <div className="mt-5 flex flex-col gap-3 rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between sm:px-5">
          <div>
            <p className="text-sm font-medium text-slate-900">
              {canContinue ? 'You\u2019re all set' : 'Two steps left'}
            </p>
            <p className="mt-0.5 text-sm text-slate-500">
              {canContinue
                ? 'Your photo and documents are verified. Continue to your dashboard.'
                : 'Finish both sections above to unlock the next step.'}
            </p>
          </div>
